...
def escape_milvus_string(value: str) -> str:
    ...
    return value
"""
Step 1: 准备多路搜索请求（AnnSearchRequest）
Step 2: 选择重排序策略（WeightedRanker）
Step 3: 执行混合搜索（hybrid_search），输出最终结果
"""
...
def create_hybrid_search_requests(dense_vector,
    ...
    dense_req = AnnSearchRequest(
        data=[dense_vector], # 搜索向量数据
        anns_field="dense_vector", # 想要对比的目标向量字段
        param=dense_params, # 搜索参数
        expr=expr, # 搜索过滤表达式
        limit=limit # 返回结果数量
    )
...
def hybrid_search(client, #MilvusClient实例
                  collection_name, # 集合名称
                  reqs,  # 搜索请求列表，固定为[dense_req, sparse_req]
                  ranker_weights=(0.5, 0.5),  # 混合搜索权重，默认(0.5,0.5)，依次对应稠密/稀疏向量
                  norm_score=True, # 是否归一化评分后再融合
                  limit=5,# 返回结果数量，默认5
                  output_fields=None, # 返回字段列表，默认返回item_name
                  search_params=None): # 搜索参数，如ef/topk等，默认None
    """
    执行Milvus稠密+稀疏向量混合搜索
    ...
    try:
        ...
        res = client.hybrid_search(
            ...
            reqs=reqs, # 搜索请求列表，固定为[dense_req, sparse_req]
            ranker=rerank, # 加权排名器：按权重融合稠密/稀疏向量的搜索结果
            limit=limit,# 返回结果数量，默认5
            output_fields=output_fields, # 返回字段列表，默认返回item_name
            search_params=search_params # 搜索参数，如ef/topk等，默认None
        )
