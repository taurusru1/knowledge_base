...
from processor.query_processor.promote.item_name_confirm import ITEM_NAME_EXTRACT_TEMPLATE, \
...
from utils.mongo_history_utils import save_chat_message, get_recent_messages, update_message_item_names
...
class NodeItemNameConfirm(NodeBase):
    ...
    def process(self, state: QueryGraphState) -> QueryGraphState:
        ...
        # 前面 step4 做的是从用户问题里提取商品名：但这个名字可能不是知识库里的标准商品名。然后 step5 去 Milvus 里搜相似商品，返回类似
        # 第 6 步就是判断这个检索出的结果是否是可信的
        align_result = {}
        if len(item_names) > 0:
    ...
    def _step_8_write_history(self, state, session_id, rewritten_query, message_id):
        """
         ...
         包含2个核心操作：1. 写入助手答案（ai给出的）（若有）；2. 更新用户原始问题的关联信息
    ...
    def _step_5_vectorize_and_query(self, item_names) -> List[Dict]:
        ...
        embeding返回的格式：
                {
                    "dense": [
                        [0.012, -0.084, 0.235, ...],   # 第 0 个商品名的 1024 维稠密向量
                        [0.031, 0.102, -0.017, ...],   # 第 1 个商品名的 1024 维稠密向量
                    ],
                    "sparse": [
                        {123: 0.82, 4567: 0.31},       # 第 0 个商品名的稀疏向量
                        {88: 0.74, 9201: 0.22},        # 第 1 个商品名的稀疏向量
                    ]
                }
        ...
        [
            {
                ...
                "matches": [
                    {"item_name": "Brother HAK180 烫金机", "score": 0.93},
                    {"item_name": "Brother HAK181 烫金机", "score": 0.72},
                    {"item_name": "RS12 万用表", "score": 0.31}
                ]
            }
        ]
        """
    ...
    def _step_4_extract_info(self, query, history) -> Dict:
        ...
        try:
            ...
            # 4.调用大模型，传入用户提示词和系统提示词，
            # llm返回的是str类型的数据，如果想要取出数据，必须要变成字典类型
            response = chat_model.invoke(messages)
            # 5.进行数据的清洗，清洗```json 为首 和```为尾 的格式
...
if __name__ == "__main__":
    ...
    # indent=4          → 格式化缩进
    json_state =  json_dumps(result, ensure_ascii=False, indent=4)
    # 输出
