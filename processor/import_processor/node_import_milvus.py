...
class NodeImportMilvus(BaseNode):
    ...
    def _step_4_insert_data(self, client, chunks_json_data):
        ...
        # 3. 回填chunk_id
        # inserted_ids为Milvus生成的自增主键列表，所以这里直接使用索引进行回填给item，从而后面可以根据id进行精确检索
    ...
    def _step_3_clean_old_data(self, client, chunks_json_data):
        ...
        """
        # 1. 获取查询条件
        # 当前这一次入库的 chunks_json_data 都来自同一份文档。所以这个chunk中的file_title都是相同的，取任何一个都是一样的
    ...
    def _clear_chunks_by_file_title(self, client, file_title):
        ...
        try:
            # 对字段进行转义处理，避免Milvus解析filter报错，避免出现字符错误从而导致报错
            file_title = escape_milvus_string(file_title)
            # 执行幂等性清理数据
            client.delete(
                # 在kb_chunks集合中清理 file_title为file_title的记录
                collection_name=milvus_config.chunks_collection,
    ...
    def _step_1_check_input(self, state: Dict[str, Any]) -> tuple[List[Dict[str, Any]], int]:
        ...
        核心校验项：
            ...
            3. 提取向量维度，为集合创建/索引构建提供依据（上一步node_bge_embending传入的）
        参数：
