...
from processor.config import ImportConfig, get_config
from processor.exceptions import ImportProcessError
...
def setup_logging(level: int = logging.INFO):
    ...
    #getLogger()里面可以传入名字，默认的名字是root
    logger = logging.getLogger()
    logger.setLevel(level)

    #定义日志记录器的颜色和格式
    # 让ai生成一个基于colorlog的日志配置
    handler = colorlog.StreamHandler() #默认走staderr
